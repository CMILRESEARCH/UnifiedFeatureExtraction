...
def regions_in_mask(Annotations, bounds, verbose=1):
    ...
    for Annotation in Annotations: # for all annotations
        ...
        for Region in Annotation['annotation']['elements']: # iterate on all region
            ...
            if verbose != 0:
                sys.stdout.write('TESTING: ' + 'Annotation: ' + str(annotationID) + '\tRegion: ' + Region['id'])
                sys.stdout.flush()
                restart_line()
...
def get_vertex_points(Annotations, IDs, verbose=1):
    ...
    for ID in IDs: # for all IDs
        print(str(ii) + '--' + str(len(IDs)))
        ii += 1
        if verbose != 0:
            sys.stdout.write('PARSING: ' + 'Annotation: ' + str(ID['annotationID']) + '\tRegion: ' + ID['regionID'])
            sys.stdout.flush()
            restart_line()
